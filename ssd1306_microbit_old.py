...
class SSD1306_I2C:
    ...
    def write_cmd(self, cmd):
        ...
        i2c.write(self.addr, bytearray([0x00, cmd]))
    ...
    def show(self):
        ...
        for i in range(0, len(self.buffer), chunk):
            tosend = bytearray(1 + min(chunk, len(self.buffer) - i))
            tosend[0] = 0x40
            tosend[1:] = self.buffer[i:i + (len(tosend) - 1)]
            i2c.write(self.addr, tosend)
            # small delay to avoid overloading the I2C bus
            sleep(0)
